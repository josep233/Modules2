...
class Test_evaluateGaussLegendreQuadrature( unittest.TestCase ):
    ...
    def test_integrate_ex( self ):
        fun = lambda x: x**3 - (8/5)*x**2 + (3/5)*x
        integrand = lambda x: fun(COB.affineMapping(x,[-1,1],[0,1]))
        actual = 1 / 60
        calculated = evaluateGaussLegendreQuadrature(integrand=integrand,degree=2,domain=[0,1])
